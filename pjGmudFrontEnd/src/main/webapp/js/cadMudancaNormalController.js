	...
	inlineCheckbox2.addEventListener('click', ()=>{
	  ...
	  if(inlineCheckbox2.checked) {
		...
		let durVonvert          = convertHoraMinutos(DuracaoTafefMinutos);
...
//		alert('index: ' + index);
        if( index === 0 ){
			if(validaStep('.step_1_validar')){
			   mudancaNormal.salvarMudancaNormal();
               $(".step:visible").hide().next().show();
               passoexibido();
            }
		}else if( index === 1 ){
			if(!mudancaNormal.validacaoAprovadores()){
               $(".step:visible").hide().next().show();
               passoexibido();
            }else MensagemConfimacao( "warning", "Validação de Campos", "Favor selecionar um Aprovador!" ) ;
			
		}else if( index === 2 ){
			if( mudancaNormal.validacaoClientesAfetados() ){
			   MensagemConfimacao( "warning", "Validação de Campos", "Favor selecionar pelo menos um Clientes Afetado pela Mudança!" );
			}else if( mudancaNormal.validacaoPlanoComunicacoes() ){
			...
			if( mudancaNormal.validacaoTarefas() ){
			...
			if(validaStep('.step_5_validar')){
			   mudancaNormal.salvarAcaosPosAtividades();
			   $(".step:visible").hide().next().show();
               passoexibido();
            }
		}else{
		   if( index === 5 ) mudancaNormal.mostraResumoTela();
           $(".step:visible").hide().next().show();
           passoexibido();			
		}
 ...
 function validaStep( stepValidate ){
	let valid = true;
	let formulario = document.querySelector('.formCadMudancaNormalValidacao');
...
//		url : "http://localhost:8090/PortalMudanca/listaImpactoMudanca",
		...
		MensagemConfimacao( "error", "Impacto Mudanca", "Erro ao Listar informacoes de Impacto Mudanca: " + xhr.responseText );
	});		
}
/******************************************************************/
/*                                                                */
...
//		url : "http://localhost:8090/PortalMudanca/listaClientesAfetados",
		...
		success : function(json) {
			...
			mudancaNormal.limparListaClientesAfetados();
			for(var p = 0; p < json.length; p++){
				let clientesAfetado = {};
				clientesAfetado.idCliAfetados = mudancaNormal.idCliAfetados;
				...
				mudancaNormal.adicionarClientesAfetado(clientesAfetado);
			    
			}
			mudancaNormal.listaTabelaClientesAfetado( mudancaNormal.listaClientesAfetados );
...
//		url : "http://localhost:8090/PortalMudanca/listaAprovador",
		...
		MensagemConfimacao( "error", "Aprovador", "Erro ao Listar Aprovador: " + xhr.responseText );
	});		
}
...
//		url         : "http://localhost:8090/PortalMudanca/obterAprovador/" + id_Aprovadores,
		...
		success     : function(response) {
 			...
 			// alert("Erro ao Listar Mudanças: " + xhr.responseText); 
			MensagemConfimacao( "error", "Aprovador", "Erro ao Selecionar Aprovador: " + xhr.responseText );
  	 }); 
}
/******************************************************************/
/*                                                                */
