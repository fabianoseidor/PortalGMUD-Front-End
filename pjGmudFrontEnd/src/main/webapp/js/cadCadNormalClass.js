...
class MudancaNormal{
	...
	salvarAcaosPosAtividades(){
		this.adicionarAcaosPosAtividadesNormall();
	}
	
	// Adiciona as informacoes da tela no Objeto
	adicionarAcaosPosAtividadesNormall(){
	...
	// Chama a API para salvar a Mudança Emergencial na base de Dados.
	salvarMudancaNormal(){
		this.adicionarMudancaNormal();
	}
	
	// Adiciona as informacoes da tela no Objeto
	adicionarMudancaNormal(){
	...
	lerDadosIdentificacaoMudanca(){
        ...
        dadosIdentificacaoMudanca.idTipoMudanca            = "1"                                                           ;
        dadosIdentificacaoMudanca.tipoMudanca              = 'Normal'                                                      ;
...
//		   alert(msg);
	...
	listaTabelaAprvadores( listaAprovadores ){
		...
		for(let i = 0; i < listaAprovadores.length; i++){
            ...
            imgDelete.setAttribute('onclick','mudancaNormal.deleteAprovadores( ' +  listaAprovadores[i].idAprov + ' )');
	...
	validaClientesAfetado(clientesAfetado){
        ...
        if(msg != null){
		   // alert(msg);
		   this.MensagemConfimacao( "warning", "Seleção de Cliente Afetado", msg ) ;
           return false;
        }
	...
	listaTabelaTarefa( tarefas ){
		...
		for(let i = 0; i < tarefas.length; i++){
            ...
            imgDelete.src = this.getContextPath() +'/imagens/delete-40.png';
			imgDelete.setAttribute('style', 'cursor:pointer;' );
            imgDelete.setAttribute('onclick','mudancaNormal.deleTetarefa( ' +  tarefas[i].idTarefa + ' )');
            td_delete.appendChild(imgDelete);
	...
	validaPlanoComunicacao(planoComunicacoe){
        ...
        if(msg != null){
		   //alert(msg);
		   this.MensagemConfimacao( "warning", "Plano Comunicação", msg ) ;
           return false;
        }
	...
	listaTabelaPlanoComunicacoe( listaPlanoComunicacoes ){
		...
		for(let i = 0; i < listaPlanoComunicacoes.length; i++){
            ...
            imgDelete.src = this.getContextPath() +'/imagens/delete-40.png';
			imgDelete.setAttribute('style', 'cursor:pointer;' );
            imgDelete.setAttribute('onclick','mudancaNormal.deletePlanoComunicacoe( ' +  listaPlanoComunicacoes[i].idPlanoComunic + ' )');
            td_delete.appendChild(imgDelete);
...
*/	
	...
	listaTabelaArquivo( listaArquivo ){
		...
		for(let i = 0; i < listaArquivo.length; i++){
            ...
            imgDelete.src = this.getContextPath() +'/imagens/delete-40.png';
			imgDelete.setAttribute('style', 'cursor:pointer;' );
            imgDelete.setAttribute('onclick','mudancaNormal.deleteArquivo( ' +  listaArquivo[i].idArqsMudanca + ' )');
            td_delete.appendChild(imgDelete);
...
//	    console.log(  JSON.stringify( dados ) );
        ...
        let urlAPI  = urlBase + 'salvarMudancaNormal';
		
		let caminhoUrlPricipal = this.getContextPath() + "/principal/telaPrincipal.jsp";
 	
		$.ajax({
			method : "POST",
//			url    : "http://localhost:8090/PortalMudanca/salvarMudancaNormal",
			url    : urlAPI,
			...
			success : function(response) {
				 ...
				 Swal.fire({
				   title: "Cadastro GMUD",
				   text : "GMUD Normal( "+ response.titulo_mudanca +" ), Cadastrada com sucesso!",
				   icon : "success",
			...
			//alert("Erro ao salvar GMUD: " + xhr.responseText);
			 $("#box_dark").css('display','none');
			if (xhr.status == 500){
...
var mudancaNormal = new MudancaNormal() 